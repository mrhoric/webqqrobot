...
VFWebQQ = ''
AdminQQ = ['1002124230']
againstmsg = [u'操你妈','你全家', '共产党', '操你', '草你', '日你', '王八蛋', '狗日的', '网络兼职', '双飞', '政治', '暴力', '黄色', '兼职', '打个小广告', '分期', '0首付', '皇冠店铺', '一夜情', '小时工', '煞笔', '祼', '借号', '卖号', '尼玛', '草你', '二逼', '妈B', '妈逼', '刷钻', '煞笔', '草拟妈', '草拟玛', '炫酷', '酷炫', '聊骚', '求赞', '黄网', '卧槽', '群发器', '豪迪QQ', '我要作死', '一天保底', '名兼职', '我妹的', '在家上班', '皇冠店铺', '日收入可到', '客服q号', '情请咨询', '兼职工资', '客服QQ', '薪金保底', '元/天', '主管QQ', '靠你的逼', '脑子有问题']
tulingkey=''
GroupL = []
GroupInfo = []
...
def getGroupList(uin):
    mGroupList = json.loads(HttpClient_Ist.Post('http://s.web2.qq.com/api/get_group_name_list_mask2','r:{"vfwebqq":"{0}","hash":"{1}"}'.format(VFWebQQ, qqhash(uin, PTWebQQ)), Referer))
    if mGroupList['retcode'] == 0:
        return mGroupList
    else:
        return False

def getGroupInfo(qcode, t):
    GroupInfo = json.loads(HttpClient_Ist.Get('http://s.web2.qq.com/api/get_group_info_ext2?gcode={0}&vfwebqq={1}&t={2}'.format(qcode, VFWebQQ, t), Referer))
    if GroupInfo['retcode'] == 0:
        return GroupInfo
    else:
        return False

def getStrangerInfo(tuin, group, t):
    StrangerInfo = json.loads(HttpClient_Ist.Get('http://s.web2.qq.com/api/get_stranger_info2?tuin={0}&verifysession=&gid=0&code=group_request_join-{1}&vfwebqq={2}&t={3}'.format(tuin, group, VFWebQQ, t), Referer))
    if StrangerInfo['retcode'] == 0:
        return StrangerInfo
    else:
        return False

def group_shutup(guin, uin, time):
    data = (
        ('gc',guin),
        ('shutup_list','[{\\"uin\\":{0},\\"t\\":{1}}]'.format(uin, time)),
        ('bkn',bkn(HttpClient_Ist.getCookie('skey'))),
        ('src','qinfo_v2')
        )
    shutup = HttpClient_Ist.Post('http://qinfo.clt.qq.com/cgi-bin/qun_info/set_group_shutup', data, 'http://qinfo.clt.qq.com/cgi-bin/qun_info/set_group_shutup')
    

def group_getout(gid, uid):    
    data = (
        ('gc',gid),
        ('ul',uid),
        ('bkn',bkn(HttpClient_Ist.getCookie('skey')))
        )
    leave = json.loads(HttpClient_Ist.Post('http://qinfo.clt.qq.com/cgi-bin/qun_info/delete_group_member', data, Referer))
    if leave['ec']:
        return False
    else:
        return True
...
lastrequest = ''        
def group_request_join(msg):
    global lastrequest
    if lastrequest == msg['value']['msg_id']:
        return
    lastrequest = msg['value']['msg_id']
    ts = time.time()
    while ts < 1000000000000:
        ts = ts * 10
    ts = int(ts)
    info = json.loads(HttpClient_Ist.Get('http://d.web2.qq.com/channel/op_group_join_req?group_uin={0}&req_uin={1}&msg=&op_type=2&clientid={2}&psessionid={3}&t={4}'.format(msg['value']['from_uin'], msg['value']['request_uin'], ClientID, PSessionID, ts), Referer))
    if info['retcode'] == 0:
        #GroupL = getGroupList(qqhash(msg['value']['t_gcode'])
        #GroupInfo = getGroupInfo(msg['value']['gcode'], ts)
        StrangerInfo = getStrangerInfo(msg['value']['request_uin'], msg['value']['from_uin'], ts)
        content = u'{0},你好，欢迎加入本群,请遵守群规！'.format(StrangerInfo['result']['nick'])
        reqURL = "http://d.web2.qq.com/channel/send_qun_msg2"
        data = (
            ('r', '{{"group_uin":{0}, "face":564,"content":"[\\"{4}\\",[\\"font\\",{{\\"name\\":\\"Arial\\",\\"size\\":\\"10\\",\\"style\\":[0,0,0],\\"color\\":\\"000000\\"}}]]","clientid":"{1}","msg_id":{2},"psessionid":"{3}"}}'.format(msg['value']['from_uin'], ClientID, msgId, PSessionID, str(content.replace("\\", "\\\\\\\\").replace("\n", "\\\\n").replace("\t", "\\\\t")).decode("utf-8"))),
            ('clientid', ClientID),
    ...
    for msg in msgObj:
        ...
        if msgType == 'group_message':
            ...
            if str(gid) in GroupWatchList:
                ...
                if g_exist:
                    g_exist.handle(tuin, txt, seq, gid)
                else:
                    tmpThread = group_thread(guin)
                    tmpThread.start()
                    GroupThreadList.append(tmpThread)
                    tmpThread.handle(tuin, txt, seq, gid)
                    logging.info("群线程已生成")
            else:
        ...
        if msgType == 'sys_g_msg':
            if msg['value']['type'] == 'group_request_join':
                group_request_join(msg)
                
                #group_shutup(msg['value']['from_uin'], msg['value']['request_uin'], ts)
...
class group_thread(threading.Thread):
    ...
    def blacklist(self, msg, uid, gid):
        for m in againstmsg:
            #self.reply(msg.decode('UTF-8'))
            if m.decode('UTF-8') in msg.decode():
                
                if self.blist.has_key(uid):
                    self.blist[uid] += 1
                    if self.blist[uid] >= 5:
                        group_getout(gid, uid)
                    else:
                        #self.reply('111')
                        self.reply('用户:{0}, 你已经违反群规{1}次, 5次将送上飞机票, 请注意言论'.format(uid, self.blist[uid]))
                else:
                   self.blist[uid] = 1
                   #self.reply('2222222')
                   self.reply('用户:{0}, 你已经违反群规{1}次, 5次将送上飞机票, 请注意言论'.format(uid, self.blist[uid]))
    ...
    def handle(self, send_uin, content, seq, gid):
        # 避免重复处理相同信息
        if seq != self.lastseq:
            self.lastseq = seq
            uid = uin_to_account(send_uin)
            self.blacklist(content, uid, gid)
            pattern = re.compile(r'^(?:!|！)(learn|delete) {(.+)}{(.+)}')
            match = pattern.match(content)
            if match and str(uid) in AdminQQ:
                if match.group(1) == 'learn':
                    self.learn(str(match.group(2)).decode(), str(match.group(3)).decode())
                    logging.debug(self.replyList)
                if match.group(1) == 'delete':
                    self.delete(str(match.group(2)).decode(), str(match.group(3)).decode())
                    logging.debug(self.replyList)
                ...
                if self.aboutme(content):
                    return
                if str(uid) in AdminQQ and self.deleteall(content):
                    return
                if self.callout(send_uin, content):
                    return
                if str(uid) in AdminQQ and self.follow(send_uin, content):
                    return
                if self.tucao(content):
                    return
                if str(uid) in AdminQQ and self.repeat(content):
                    return
    ...
    def save(self):
        try:
            ...
            with open("blacklist.save", "w+") as savefile2:
                savefile2.write(json.dumps(self.blist))
                savefile2.close()                
        except Exception, e:
    ...
    def load(self):
        try:
            ...
            with open("blacklist.save", "r") as savefile2:
                saves2 = savefile2.read()
                if saves2:
                    self.blist = json.loads(saves2)
                savefile2.close()
